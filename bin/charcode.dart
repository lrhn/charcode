...
///
/// `-H`/`--html` includes declarations for HTML entity names.
///
...
void main(List<String> args, [StringSink? output]) {
  ...
  if (outputFile == null) {
    ...
    var start = 0;
    var end = 0;
    var startComment = "// $insertionMarkerStart\n";
    var endComment = "// $insertionMarkerEnd\n";
    if (file.existsSync()) {
      content = file.readAsStringSync();
      var nl = findNewline(content);
      startComment = "// $insertionMarkerStart$nl";
      start = content.indexOf(startComment);
      if (start >= 0) {
        endComment = "// $insertionMarkerEnd$nl";
        end = content.indexOf(endComment, start);
        if (end >= 0) {
    ...
    var buffer = StringBuffer(startComment);

    buffer.writeln("// Generated by");
    buffer.write("// dart run charcode ");
    writeCommandLine(buffer, args);
    buffer.writeln();
    buffer.writeln();

    declarations.writeTo(buffer);

    buffer.writeln();
    buffer.write(endComment);
    var newContent = content.replaceRange(start, end, buffer.toString());
    if (content == newContent) {
      warn("No change to generated file");
    } else {
...
const String insertionMarkerStart = "START GENERATED CHARCODES";
const String insertionMarkerEnd = "END GENERATED CHARCODES";

String findNewline(String text) {
  var index = text.indexOf("\n");
  if (index > 0 && text.startsWith("\r", index - 1)) return "\r\n";
  return "\n";
}

void writeCommandLine(StringSink output, List<String> args) {
  var nonSimpleRE = RegExp(r'[^a-zA-Z_0-9/.\-=]');
  var sep = "";
  for (var arg in args) {
    output.write(sep);
    sep = " ";
    if (nonSimpleRE.hasMatch(arg)) {
      if (arg.contains("'")) {
        output
          ..write('"')
          ..write(arg.replaceAll("'", r"\'").replaceAll('"', r'\"'))
          ..write('"');
      } else {
        output
          ..write("'")
...
void warn(String warning) {
  if (verbose > 0) stderr.writeln(warning);
}
...
class CharacterDeclaration {
  /// The un-prefixed name to use for the code point.
  final String name;
...
/// and accumulates requested-for declarations for a number
/// of those.
...
Usage:
  ...
  Emits constant declarations for the characters specified by character ranges.
